...
- If you have a `.exe` file, double-click it to open.
...
3. **Execute**: Does the action the instruction calls for.
4. **Memory**: Reads or writes data to memory.
5. **Write-back**: Saves the results back.
...
## 🛠️ Installing Prerequisites (If Needed)
...
- A user guide in `.pdf` or `.txt`
- Sample instruction files to practice running the processor
